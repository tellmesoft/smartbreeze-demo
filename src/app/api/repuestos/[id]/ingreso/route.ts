...
export async function POST(request: Request, { params }: Params) {
  ...
  try {
    ...
    const descontarPedido = Math.min(cantidad, repuesto.cantidadPedida);
    const cantidadPedida = repuesto.cantidadPedida - descontarPedido;
    ...
    await prisma.$transaction([
      ...
      prisma.movimientoRepuesto.create({
        data: {
          ...
          cantidad,
          cantidadResultante,
          observaciones: observaciones ?? "Ingreso de stock a bodega.",
